...
// 初始化搜索引擎图�?setEngine(currentEngine);
...
function initSortable() {
    ...
    Sortable.create(DOM.bookmarksSection, {
        animation: 150,
        handle: '.group-header', // 仅可通过标题栏拖�?        ghostClass: 'sortable-ghost',
    ...
    document.querySelectorAll('.sites-grid').forEach(grid => {
        Sortable.create(grid, {
            ...
            onEnd: async function (evt) {
                ...
                // 无论是同组排序还是跨组，都需要更新目标组内的所有元素顺�?                const siteCards = evt.to.querySelectorAll('.site-card');
...
DOM.btnUpgrade.addEventListener('click', async () => {
    ...
    DOM.btnUpgrade.textContent = '升级�?..';
    DOM.upgradeLog.classList.remove('hidden');
    DOM.upgradeLog.innerHTML = '<div class="log-item">正在向服务器发送升级指�?..</div>';
    
    try {
        const res = await api.settings.upgrade();
        DOM.upgradeLog.innerHTML += `<div class="log-item"><pre>${res.log || '升级已完成，无日志返�?}</pre></div>`;
...
// ── 初始化入�?────────────────────────────────
async function init() {
    // 先检查注册状�?    const token = localStorage.getItem('navi_token');
    ...
    try {
        ...
        if (token) {
            ...
            // 还没有用�?�?跳转注册�?            location.replace('/register');
        } else {
            // 已有用户但没 token �?跳转登录�?            location.replace('/login');
        }
    } catch (err) {
        // 连不上服务器
